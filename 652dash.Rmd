...
## CHRIS' COMMENT
## I added a \n within your longer label, just to give the plot more space
...
### Text to Explain Plot

Adding words here would help people understand the final plot a bit more:
  * Background 
  * Sample Characteristics
  * What you want the reader to draw from your plot
  * Something to prep you for the next pages
...
## CHRIS' COMMENT
## coord_flip() can be bypassed these days with manually specifying the x and y.
## Also, I assume you don't want the FSDHH == NA plotted, when I made the plot
## it looked like a labeled "5" with no bar, so I filtered those for missingness 
## I also labeled your factor in the mutate call so you only did it once
## this could be my eyes, but that light green is kind of hard to see. The 
## darker green and blue are okay, but I have some trouble seeing that light green

secure_group %>% 
  filter(!is.na(FSDHH)) %>% 
  ggplot() +
  geom_col(aes(y = FSDHH, 
               x = dpq_group, 
               fill = FSDHH), 
           alpha = .8,
           show.legend = F) +
  geom_text(aes(y = FSDHH, 
                x = dpq_group, 
                label = paste0(round(dpq_group,2))), 
            nudge_x = .4, 
            size = 4) +
...
```{r v2secure}
## CHRIS' COMMENT
## See other edits above to the final draft

#removing NA group
#secure_group <- secure_group[-5,]
## I did this above for you via the %>% filter(!is.na(FSDHH))
...
full_yr_grouped %>%
  ...
  geom_text_repel(data = filter(full_yr_grouped, year == "2015"),
            aes(y = ed_mean, label = FSDHH, color = FSDHH),
            show.legend = F,
            nudge_x = 5, 
...
## CHRIS' COMMENT
## I reorderd these so you are presenting rows in their (average) frequencies
## I did the same for the facet_wrapping.
## Maybe consider some font size changes;
## I didn't think this was a good one to magnify all
