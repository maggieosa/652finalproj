...
secure_group %>%
  ggplot() +
  geom_col(aes(FSDHH,dpq_group), fill = "thistle", alpha = .8) +
...
```{r plot3, fig.height=10, fig.width=4}
#recoding factors 
...
```{r datacal}
cal <- here::here("data", "2017_individual_day_nutrition.XPT") %>%
  read_xpt()

dep17 <- here::here("data", "2017_depression_screener.XPT") %>%
  read_xpt()

total_cal <- left_join(dep17,cal)

demo17 <-here::here("data", "2017_demographics.XPT") %>%
  read_xpt()

weight17 <- here::here("data", "2017_weight_history.XPT") %>%
  read_xpt()

df_cal <- left_join(total_cal, demo17)

df_cal_1 <- left_join(df_cal,weight17)
#create depression composite
new_cal <- df_cal_1 %>%
  rowwise() %>%
  mutate(dpq_sum = sum(c(DPQ010, DPQ020, DPQ030, DPQ040, DPQ050,
                        DPQ060, DPQ070, DPQ080,DPQ090, DPQ100)),
         cals = DR1TKCAL/WHD010) %>%
  drop_na(c(dpq_sum, cals)) %>%
  filter(RIAGENDR == 2)

new_cal %>%
  ggplot(aes(cals, dpq_sum))+
  geom_point(alpha = 0.2) +
  geom_smooth()

mod2<- lm(dpq_sum ~cals, data = new_cal)
summary(mod2)
